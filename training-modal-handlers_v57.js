 ...
 * @param {object} event - The FullCalendar event object that was clicked.
 */
function launchEditModal(event) {
    // --- THIS IS THE INTEGRATED FIX ---

    const eventDataForModal = event.originalData;
    // --- END OF FIX ---

    console.log("launchEditModal called for event:", eventDataForModal);
    currentEventForEdit = eventDataForModal; // Store the full data object

    const $modal = $("#editEvent");
    const modalStartDate = moment(eventDataForModal.start);
    const modalInclusiveEndDate = moment(eventDataForModal.end).subtract(1, 'day');

    // --- 1. Prepare Modal UI ---
    // Use the event's original title if available, otherwise use the simple one
    const displayTitle = eventDataForModal.title || event.title || "N/A";
    ...
    $changeBtn.off('click').on('click', function () {
        ...
        updateTrainingEvent(eventDataForModal.id, eventDataForSave)
            .then(response => {
    ...
    $removeBtn.off('click').on('click', function () {
        ...
        removeTrainingEvent(eventDataForModal.id)
            .then(response => {
    ...
    // Pass the full eventDataForModal object to the loader
    loadEditModalData(eventDataForModal, modalStartDate, modalInclusiveEndDate);
 ...
 */
function loadEditModalData(event, start, end) {
...
function launchTrainerEditModal(event) {
    console.log("launchTrainerEditModal called for event:", event);
...
function launchModal(start, end) {
    ...
    // --- THIS IS THE FIX ---
    // 1. Calculate the TRUE inclusive end date at the very beginning.
    const inclusiveEndDate = end.clone().subtract(1, 'day');
    // --- END OF FIX ---

    // 2. Pass the corrected dates to all helper functions.
