$(document).ready(function () {
    ...
    recognition.maxAlternatives = 3; // Increased to capture more alternatives
    ...
    recognition.onresult = function (event) {
        ...
        if (final_transcripts.length > 0 && isListening) {
            ...
            console.log("Data being sent:", { command: commandText, step: currentStep });
    ...
    recognition.onerror = function (event) {
        ...
        const message = event.error === 'no-speech'
            ? `AI: No speech detected. Please click the button and speak again for the ${currentStep || 'command'}.`
    ...
    function processVoiceCommand(commandText) {
        ...
        $.ajax({
            ...
            data: JSON.stringify({ command: commandText, step: currentStep }),
            dataType: 'json'
        })
    ...
    function handleAiResponse(response) {
        ...
        // === NEW, IMPROVED CODE with DELAY and better UI ===
        if (response.status === 'next') {
            currentStep = response.nextStep;

            const notyMessage = `AI: ${response.message}`;
            new Noty({ text: notyMessage, type: 'info', timeout: 7000 }).show(); // Longer timeout for the message
            speakMessage(response.message);

            // Give the user a clear instruction on the button itself.
            aiButtonText.text(`Waiting for ${currentStep.replace('confirm_', '')}...`);

            // Give the user a few seconds to think before opening the mic.
            setTimeout(() => {
                // Check that the conversation hasn't been cancelled in the meantime.
                if (currentStep && !isListening) {
                    console.log("AI Assistant: Automatically activating microphone for next step.");
                    aiButton.click(); // Automatically "click" the button to start listening.
                }
