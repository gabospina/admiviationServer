<?php
// hangar.php - FINAL DYNAMIC VERSION
...
// =========================================================================
// === START: CORRECTED DYNAMIC DATA FETCHING LOGIC                      ===
// =========================================================================

// --- 1. Fetch the STANDARD list of fields for the user's company ---
$standardFields = [];
// --- THIS IS THE FIX: Corrected the table name ---
...
// --- 2. Fetch the SPECIFIC validity data for THIS pilot ---
$pilotValidityData = [];
if (!empty($standardFields)) {
    // Build a list of columns to select based on the standard fields
    $columnsToSelect = [];
    foreach (array_keys($standardFields) as $key) {
        // Ensure the key is safe before adding it to the query
        if (preg_match('/^[a-zA-Z0-9_]+$/', $key)) {
            $columnsToSelect[] = "`" . $key . "`";       // e.g., `passport`
            $columnsToSelect[] = "`" . $key . "_doc`";   // e.g., `passport_doc`
        }
    }

    if (!empty($columnsToSelect)) {
        $sqlData = "SELECT " . implode(', ', $columnsToSelect) . " FROM user_licences_data WHERE user_id = ?";
        ...
        if ($stmtData) {
            ...
            if ($resultData->num_rows > 0) {
                $pilotValidityData = $resultData->fetch_assoc();
            }
            $stmtData->close();
        } else {
             error_log("Hangar.php - Failed to prepare statement for user_licences_data: " . $mysqli->error);
        }
    }
}

// =========================================================================
// === END: CORRECTED DYNAMIC DATA FETCHING LOGIC                        ===
// =========================================================================
                            ...
                            <tr data-field="<?= htmlspecialchars($field_key) ?>">
                                ...
                                <td>
                                    <div class="input-group" style="width: 170px; position: relative; z-index: 1;">
                                        <input type="text" class="form-control datepicker validity-date" value="<?= htmlspecialchars($expiryDate) ?>" placeholder="MM-DD-YYYY">
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <select id="clock-timezone" class="form-control">
                                <?php 
                                // This PHP block to generate timezones is correct
                                ...
                                foreach ($timezones as $tz) {
                                    ...
                                    $sign = $offset >= 0 ? '+' : '';
                                    echo "<option value='$tz' data-offset='$offset'>(UTC$sign$offset) $tz</option>";
                                }
                                ?>
                            </select>
                        </div>
