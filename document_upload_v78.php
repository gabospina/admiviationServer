...
try {
    ...
    if (!empty($newCategoryNameProvided)) {
        ...
        if ($resultCheck->num_rows > 0) {
            ...
            $stmtInsert = $mysqli->prepare("INSERT INTO document_categories (category, created_by) VALUES (?, ?)");
            if (!$stmtInsert) { throw new Exception("Prepare failed (insert category): " . $mysqli->error); }
            $stmtInsert->bind_param("si", $categoryName, $userId);
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (empty($fileExtension) || !in_array($fileExtension, $allowedExtensions)) {
            ...
            if (move_uploaded_file($file['tmp_name'], $destinationAbsolute)) {
                ...
                $sql = "INSERT INTO documents (original_filename, stored_filename, filepath, preview_filepath, mime_type, filesize, category_id, creator, upload_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";
