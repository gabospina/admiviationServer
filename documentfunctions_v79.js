	...
	$.ajax({
		...
		success: function (response) {
			...
			if (response?.success && response.data) {
				...
				if (pathForPdfEmbed) {
					console.log("DEBUG getDocument: Generating PDF preview for:", pathForPdfEmbed);

					// === ENHANCEMENT: DUAL PDF VIEWER OPTIONS ===
					const absolutePdfUrl = new URL(pathForPdfEmbed, window.location.origin).href;
					const googleViewerUrl = `https://docs.google.com/gview?url=${encodeURIComponent(absolutePdfUrl)}&embedded=true`;
					const nativePdfUrl = `${htmlspecialchars(pathForPdfEmbed)}#toolbar=1&navpanes=1&scrollbar=1&view=FitH`;
					...
					<div class="document-preview pdf-preview">
						<div class="preview-info">
							...
							<div class="btn-group btn-group-sm mt-2" role="group">
								<button type="button" class="btn btn-outline-primary active" data-viewer="google">
									<i class="fa fa-google"></i> Google Viewer
								</button>
								<button type="button" class="btn btn-outline-secondary" data-viewer="native">
									<i class="fa fa-file-pdf-o"></i> Native PDF
								</button>
							</div>
						</div>
						${previewFilePath !== originalFilePath && previewFilePath ?
						...
						<iframe 
							...
							width="100%" 
							height="700px" 
							title="Document Preview - Google Viewer"
							class="pdf-viewer-iframe"
							data-viewer-type="google"
							style="display: block;">
							<p>Your browser does not support embedded documents.</p>
						</iframe>
						
						<!-- Native PDF Viewer (Hidden by default) -->
						<iframe 
							src="${htmlspecialchars(nativePdfUrl)}" 
							type="application/pdf" 
							width="100%" 
							height="700px" 
							title="Document Preview - Native PDF"
							class="pdf-viewer-iframe"
							data-viewer-type="native"
							style="display: none;">
							<p>Your browser does not support embedded PDF documents.</p>
						</iframe>
						
						<div class="text-center mt-3">
							<a href="${htmlspecialchars(originalFilePath)}" target="_blank" class="btn btn-sm btn-outline-primary me-2">
								<i class="fa fa-external-link-alt"></i> Open in New Tab
							</a>
							<a href="${htmlspecialchars(originalFilePath)}" download="${htmlspecialchars(originalFilename)}" class="btn btn-sm btn-outline-success">
								<i class="fa fa-download"></i> Download File
							</a>
						</div>
					</div>`;
				}
				// --- END OF ENHANCED PDF BLOCK ---
				else if (pathForImageEmbed) {
					// Image Preview
					console.log("DEBUG getDocument: Generating Image preview for:", pathForImageEmbed);
					previewHtml = `
                    <div class="document-preview image-preview text-center">
                        <img src="${htmlspecialchars(pathForImageEmbed)}" class="img-fluid" style="max-height: 600px; border: 1px solid #ccc;">
                        <p class="text-center mt-2">
                            <a href="${htmlspecialchars(pathForImageEmbed)}" target="_blank" class="btn btn-sm btn-outline-primary me-2">
                                <i class="fa fa-external-link-alt"></i> Open in New Tab
                            </a>
                            <a href="${htmlspecialchars(pathForImageEmbed)}" download="${htmlspecialchars(originalFilename)}" class="btn btn-sm btn-outline-success">
					...
					const downloadButtonHtml = originalFilePath ? `
                        <div class="text-center">
                            <a href="${htmlspecialchars(originalFilePath)}" target="_blank" class="btn btn-primary me-2">
                                <i class="fa fa-external-link-alt"></i> Open in New Tab
                            </a>
                            <a href="${htmlspecialchars(originalFilePath)}" download="${htmlspecialchars(originalFilename)}" class="btn btn-success">
                                <i class="fa fa-download"></i> Download File
                            </a>
                        </div>` : '<p class="text-center text-danger">Error: Original file path is missing.</p>';
				...
				// === ADDED: PDF VIEWER SWITCHER FUNCTIONALITY ===
				if (pathForPdfEmbed) {
					// Add click handlers for PDF viewer switcher
					$('.btn-group button[data-viewer]').on('click', function () {
						const viewerType = $(this).data('viewer');

						// Update active button
						$(this).addClass('active').siblings().removeClass('active');

						// Show/hide appropriate iframe
						$('.pdf-viewer-iframe').hide();
						$(`.pdf-viewer-iframe[data-viewer-type="${viewerType}"]`).show();
					});
				}
				// --- End Preview Generation ---
