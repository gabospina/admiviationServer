...
// === FIX #2: GET COMPANY_ID FROM SESSION ===
if (!isset($_SESSION['company_id'])) {
    $response['message'] = "Authentication error: Company ID not found in session.";
    http_response_code(401);
    echo json_encode($response);
    exit;
}
...
try {
    ...
    if (!empty($newCategoryNameProvided)) {
        ...
        // === FIX #3: ADD COMPANY_ID TO CATEGORY CHECK ===
        $stmtCheck = $mysqli->prepare("SELECT id FROM document_categories WHERE LOWER(category) = LOWER(?) AND company_id = ?");
        ...
        $stmtCheck->bind_param("si", $lowerCategoryName, $company_id);
        $stmtCheck->execute();
        ...
        if ($resultCheck->num_rows > 0) {
            ...
            // === FIX #4: ADD COMPANY_ID TO CATEGORY INSERT ===
        ...
        // === FIX #5: ADD COMPANY_ID TO CATEGORY NAME FETCH ===
        $stmtGetName = $mysqli->prepare("SELECT category FROM document_categories WHERE id = ? AND company_id = ?");
         if ($stmtGetName) {
             $stmtGetName->bind_param("ii", $categoryId, $company_id);
             if ($stmtGetName->execute()) {
                 ...
                 if($resultName->num_rows > 0) {
                    ...
                    error_log("WARN: Selected category ID $categoryId not found in DB for company $company_id.");
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (empty($fileExtension) || !in_array($fileExtension, $allowedExtensions)) {
            ...
            if (move_uploaded_file($file['tmp_name'], $destinationAbsolute)) {
                ...
                // In document_upload.php, after the conversion logic, add:
                if ($fileExtension === 'pdf') {
                    // For PDF files, the original file IS the preview
                    $previewFilepathRelative = $uploadedFilepathRelative;
                    error_log("DEBUG: PDF file - using original as preview: " . $previewFilepathRelative);
                }

                // --- Insert Document Record ---
                // === FIX #6: ADD COMPANY_ID TO DOCUMENTS INSERT ===
                ...
                // Assuming filesize is INT, category_id is INT, creator is INT, company_id is INT
