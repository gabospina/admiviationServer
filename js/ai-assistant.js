...
$(document).ready(function () {
    ...
    function processVoiceCommand(commandText) {
        ...
        $.ajax({
            type: 'POST',
            url: 'daily_manager_ai_process_command.php',
            // url: 'daily_manager_ai_process_command_v51.php',
            contentType: 'application/json', // 1. Tell the server we are sending JSON
            data: JSON.stringify({ command: commandText }), // 2. Convert the JS object to a JSON string

            dataType: 'json' // We still expect JSON back
        })
            .done(function (response) {
                console.log("AI Core Response:", response);
                handleAiResponse(response);
            })
            .fail(function (jqXHR, textStatus, errorThrown) { // Added more error details
                console.error("AJAX Error:", textStatus, errorThrown);
                console.error("Response Text:", jqXHR.responseText);
                new Noty({ text: 'Error: Could not communicate with the AI Assistant service.', type: 'error', timeout: 4000 }).show();
            })
            .always(function () {
    ...
    // =====================================================================
    // 8. === THE EXECUTION ROUTER (with better validation) =================
    // =====================================================================
    function executeAiPlan(action, parameters) {
        ...
        switch (action) {
            case 'CREATE_PILOT':
                // --- THE FIX IS HERE: Stricter validation before sending ---
                // Check if we have the minimum required data extracted by the AI.
                // We now check for a valid email as well.
                if (!parameters.firstname || !parameters.username || !parameters.email) {
                    let missing = [];
                    if (!parameters.firstname) missing.push('full name');
                    if (!parameters.username) missing.push('username');
                    if (!parameters.email) missing.push('email');

                    const errorMessage = `AI: I understood "create pilot", but I missed the following required information: ${missing.join(', ')}. Please try again.`;
                    new Noty({ text: errorMessage, type: 'error', timeout: 5000 }).show();
                ...
                $.ajax({
                    ...
                    .done(function (result) {
                        if (result.success) {
                            new Noty({ text: `AI: Successfully created pilot: ${parameters.firstname} ${parameters.lastname}`, type: 'success', timeout: 3000 }).show();
                        } else {
            ...
            case 'ADD_CRAFT':
                // ... (The ADD_CRAFT case remains the same) ...
                ...
                $.ajax({
                    ...
                    if (result.success) {
                        new Noty({ text: `AI: Successfully added craft: ${parameters.registration}`, type: 'success', timeout: 3000 }).show();
                        if (typeof loadAndBuildManagerCraftsTable === 'function') {
                            loadAndBwuildManagerCraftsTable();
                        }
                    } else {
