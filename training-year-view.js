...
// Add this function to your training-year-view.js file.

/**
 * Toggles the calendar between its standard view and our custom 12-month grid.
 * @param {string} parentSelector - The selector of the calendar ('#calendar' or '#trainer-calendar').
 */
function toggleYearView(parentSelector) {
    ...
    if (CalendarState.isYearView) {
        // --- ENTERING YEAR VIEW ---
        console.log(`Entering Year View for ${parentSelector}`);

        // Update button states
        $calendar.find('.fc-year-button').addClass('fc-button-active');
        $calendar.find('.fc-dayGridMonth-button, .fc-timeGridWeek-button, .fc-timeGridDay-button').removeClass('fc-button-active');
        ...
        // Load the 12 mini-calendars if they haven't been loaded yet.
        // In v6, we check for the container's children having the 'fc' class.
        if ($(parentSelector + " .fc-month-container .fc").length === 0) {
            loadYearCalendar(parentSelector);
        }
    } else {
        // --- EXITING YEAR VIEW ---
        // When toggling OFF, just go back to the default month view.
        ...
        if (calendarInstance) {
            ...
            // The viewDidMount callback will fire and call exitYearView() to clean up the UI.
        }
    }
}
 ...
 * This is called by the viewDidMount callback when a standard view is rendered.
 */
function exitYearView1(parentSelector) {
    // In v6, the standard buttons handle their own active state.
    // We just need to deactivate our custom year button.
    $(parentSelector).find('.fc-year-button').removeClass('fc-button-active');

    // Hide the year view and its controls
    $(parentSelector).find(".fc-year-container, #year-directionals").hide();

    // Show the standard view and its controls
    $(parentSelector).find(".fc-view-harness, #non-year-directionals, .fc-toolbar-title").show();
...
function exitYearView(parentSelector) {
    ...
    // Only proceed if the year view is actually active
    if (!CalendarState.isYearView && !$calendar.find('.fc-year-button').hasClass('fc-button-active')) {
        // We are already in a standard view, nothing to do.
        return;
    }

    console.log(`Exiting Year View for ${parentSelector}`);

    $calendar.find('.fc-year-button').removeClass('fc-button-active');
